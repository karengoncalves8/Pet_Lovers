...
const Servicos = () => {
    ...
    // Fetch inicial para carregar os serviços
    const fetchServicos = async () => {
        const result = await consumivelServices.getConsumiveisByType("Servico");
    ...
    const handleClick = () => {
        ...
        const newRow: Consumivel = {
            ...
            Cons_descricao: '',
            Cons_valor: 0.00,
            Cons_tipo: "Servico", // Define o tipo como "Servico"
            isNew: true,
        };
    ...
    const processRowUpdate = async (newRow: Consumivel): Promise<Consumivel> => {
        try {
            if (newRow.isNew) {
                ...
                if (response instanceof ApiException) {
                    console.error('Erro ao criar serviço:', response.message);
                ...
                if (response instanceof ApiException) {
                    console.error('Erro ao atualizar serviço:', response.message);
                    throw new Error(response.message);
                }
